...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Depth Testing", NULL, NULL);
	if (window == NULL)
	{
		printf("Failed Craeting Window \n");
		glfwTerminate();
		return -1;
	}
    ...
    Shader ourShaderOutline("./src/vShaderSource.vs", "./src/fShaderSourceOutline.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ourShaderOutline.setMat4("model", model);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
        model = glm::scale(model, glm::vec3(scale, scale, scale));
